...
from web.views.friends.message.graph import ChatGraph
from web.views.friends.message.memory.update import update_memory
...
def add_system_prompt(state, friend):
    msgs = state['messages']
    system_prompts = SystemPrompt.objects.filter(title='reply').order_by('order_number')
    prompt = ''
